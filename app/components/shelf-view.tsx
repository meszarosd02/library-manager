...
export default function ShelfView(
    ...
    useEffect(() => {
        const asd = async () => {
            console.log(await getBookCountInShelf(1))
        }
        asd()
    }, [])
        ...
        <>
            <div className={`grid`} style={{ gridTemplateColumns: `repeat(${shelf.col_num}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${shelf.row_num}, minmax(0, 1fr))` }}>
                {Array.from({ length: shelf.col_num * shelf.row_num }).map((_, i) => (
                    <div key={i} className="border border-gray-200 p-2 h-10 cursor-pointer hover:bg-gray-600" onClick={() => onSectionClick(Math.floor(i / shelf.col_num), i % shelf.col_num)}></div>
                ))}
            </div>
        </>
    )
